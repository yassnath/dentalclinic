...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  if (ctx.req.method === "POST") {
    ...
    if (!MENUNGGU_STATUSES.includes(status)) {
      return {
        redirect: {
          destination: "/resepsionis/pasien-aktif?error=Hanya%20pendaftaran%20yang%20masih%20menunggu%20yang%20bisa%20ditolak.",
          permanent: false,
        },
      };
    }
    ...
    return {
      redirect: {
        destination: "/resepsionis/pasien-aktif?success=Pendaftaran%20berhasil%20ditolak.",
        permanent: false,
      },
    };
  }
...
export default function ResepsionisTolakActionPage() {
  return null;
}
